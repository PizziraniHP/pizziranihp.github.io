...
- Cada ID de pessoa deve existir uma unica vez no cadastro base (unicidade obrigatoria).
- Evitar duplicar a mesma pessoa em blocos diferentes quando o objetivo for apenas alterar posicao visual.
- Posicao visual deve ser controlada por bloco/referencia da arvore, nao por duplicacao de cadastro.
...
## Regra de clareza dos blocos por geracao

- Em geracoes com uma unica linha, manter bloco unico por ramo quando possivel.
- Em geracoes com mais de uma linha, usar nomes de bloco que explicitem linha e lado (ex.: linha1-esquerda, linha1-direita, linha2-esquerda, linha2-direita).
- Quando um ajuste for apenas de leitura visual (esquerda/direita, primeira/segunda linha), priorizar reorganizacao de refs e blocos de exibicao, sem criar novo cadastro de pessoa.
- Evolucao futura (plano posterior): padronizar oficialmente subdivisao por participacao para cenarios de 2 ou 3 linhas, depois que as funcoes atuais estiverem estaveis.
