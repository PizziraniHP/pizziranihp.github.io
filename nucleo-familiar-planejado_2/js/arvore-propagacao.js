(function () {
  ...
  function visualLineage(generation, rowLength, colIndex, fallbackLineage) {
    if (generation >= 4 && rowLength === 8) {
      return colIndex < 4 ? 'LP' : 'LM';
    }
  ...
  async function resolveSaibaMaisPage(basePath, personId, personName) {
    ...
    if (indexMap && indexMap[id]) {
      try {
        var mappedUrl = indexMap[id];
        var mappedRes = await fetch(mappedUrl, { cache: 'no-store' });
        if (mappedRes.ok) {
          saibaMaisCache[id] = mappedUrl;
          return saibaMaisCache[id];
        }
      } catch (err) {
        // Se o indice estiver desatualizado, cai no fallback por padroes de nome.
      }
    }
  ...
  function resolveExplicitSaibaMaisUrl(basePath, explicitPath) {
    var raw = String(explicitPath || '').trim();
    if (!raw) {
      return '';
    }
  ...
  function renderLine(cards, generation, imagePrefix) {
    ...
    rows.forEach(function (row) {
      ...
      row.forEach(function (p, colIndex) {
        var baseLineage = lineageFromCard(p, generation);
        var effectiveLineage = visualLineage(generation, row.length, colIndex, baseLineage);
        var ramoCls = effectiveLineage === 'LP' ? 'ramo-paterno' : (effectiveLineage === 'LM' ? 'ramo-materno' : 'ramo-centro');
        ...
        var detalhe = p.detalhe || (effectiveLineage === 'LP' ? 'PATERNO' : (effectiveLineage === 'LM' ? 'MATERNO' : 'CENTRO'));
        var codigo = slotCode ? ('G' + generation + '-' + effectiveLineage + '-' + slotCode) : codeFromPerson(p);
        ...
        html += [
          '<div class="card-pessoa ' + cfg.card + ' ' + ramoCls + '"' + idAttr + canonicalAttr + explicitAttr + ' data-person-name="' + (p.nome || '') + '">',
          ...
          isPlaceholder ? '' : '<div class="saiba-mais-wrap"><a class="saiba-mais-link" href="' + saibaMaisHref + '" data-saiba-mais="1"' + explicitAttr + ' data-person-id="' + personId + '" data-person-name="' + (p.nome || '') + '"><span class="saiba-mais-icone" aria-hidden="true"></span><span class="saiba-mais-texto">SAIBA MAIS</span></a></div>',
          '</div>'
        ].join('');
      });
      html += '</div>';
  ...
  window.initArvorePropagacao = async function initArvorePropagacao(opts) {
    ...
    target.querySelectorAll('.card-pessoa').forEach(function (card) {
