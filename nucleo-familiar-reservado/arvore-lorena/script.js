document.addEventListener('DOMContentLoaded', function () {
    ...
    function autoFillRamoClasses() {
        ...
        rows.forEach(function (row) {
            ...
            if (isG6Row) {
                const known = classified.filter(function (entry) { return !entry.placeholder; });
                if (!known.length) {
                    cards.forEach(function (card) { card.classList.remove('ramo-paterno', 'ramo-materno'); });
                    return;
                }
                const rowIsPartial = known.length < cards.length;
                const singleBranch = known.every(function (entry) { return entry.branch === known[0].branch; }) ? known[0].branch : '';
